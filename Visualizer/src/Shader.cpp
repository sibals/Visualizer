...
{
	this->secondaryTextureHandle = BAD_GL_VALUE;
...
bool RenderTextureShader::Initialize(char * vertex_shader_file, char * fragment_shader_file) {
	...
	this->Use();
	this->secondaryTextureHandle = glGetUniformLocation(this->program_id, (const GLchar *) "tex02");
...
void RenderTextureShader::CustomSetup(GLuint textureID, GLuint render_text) {
	glUniform1i(this->render_texture_handle, render_text);
	glUniform1i(this->secondaryTextureHandle, textureID);
